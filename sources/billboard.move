module billboard::billboard {
    ...
    public entry fun add_message(sender: &signer, message: String) acquires Billboard {
        ...
        let billboard = &mut Billboard[@billboard];
    ...
    inline fun only_owner(owner: &signer) {
        assert!(signer::address_of(owner) == @billboard, error::permission_denied(ENOT_OWNER));
    }

    #[view]
    public fun get_messages(): vector<Message> acquires Billboard {
        let billboard = &Billboard[@billboard];
    ...
    #[test(aptos_framework = @std, owner = @billboard, alice = @0x1234, bob = @0xb0b)]
